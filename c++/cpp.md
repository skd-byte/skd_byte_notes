...
- **xvalue**: An “expiring” object with identity that is about to be moved from (e.g., `std::move(v)`).
    ...
    ```c++
        ...
        unique_ptr<shape> ps{read_shape(kind::smiley,{2,4}, 4, 0)};
        //Like a for-statement, an if-statement can introduce a variable and test it.
        // ps.get();              // get raw pointer
        // p.get() returns the raw pointer managed by the unique_ptr.
        // Ownership remains with the unique_ptr, so you don’t manually delete raw.
        if (smiley* p = dynamic_cast<smiley*>(ps.get()); p != nullptr) {
            p->wink_eye();
        }
    ```
...
- copy and move all the scenarios including conversion
- implement example for implicit and explicit conversion with () = {}
- move constructor assignment with prints, copy and move for all the scenarios with print like in function call and check what compiler call move or copy
...
- there are five places where object can be copied and moved:
    - as the source of an assignment
        ```c++
        A a1(10);
        A a2(20);
        a2 = a1;        // copy assignment operator called
        ```
    - as an object initializer
        ```c++
        // 2. As an object initializer → creating a new object from another object
        A a1(10);
        A a2 = a1;      // copy constructor called
        // note:: A a = 10;     → Copy initialization → 10 converted to A(10) → constructor runs
        ```
    - as a function argument
        ```c++
        // 3. As a function argument → object passed by value to a function
        void func(A x) {}
        A a1(10);
        func(a1);       // copy constructor (or move) used
        ```
    - as a fuction return value
        ```c++
        // 4. As a function return value → returning object by value from function
        A make() {
            A temp(10);
            return temp;    // copy/move may happen (often optimized by RVO)
        }
        A a1 = make();
        ```
    - as a execption
        ```c++
        // 5. As an exception → object copied when thrown/caught by value
        try {
            A a1(10);
            throw a1;       // copy constructor used
        }
        catch(A e) { }      // another copy when catching by value
        ```

    > a copy or move constructor invocation is often optimized away by constructing the object used to initialize right in the target object.  compiler will typically construct the X from make() directly in x; thus eliminating ***(“eliding”)*** a copy
    ```c++
    X make(Sometype);
    X x = make(value);
    ```

- initialization of named objects and of objects on the free store, constructors are used to initialize temporary objects and to implement explicit
    ```c++
            class A{ public: A(int x){} };
            A a(10);         // named object initialization
            A* p = new A(5); // object created on free store (heap)
            A b = A(20);     // temporary object conceptually created (usually copy elision)
            A c = 30;        // implicit conversion: int → A using constructor
            A d = A(30);     // explicit conversion: int → A, here compiler will do the rvo
    ```
...
- Except for the “ordinary constructor,” these special member functions will be generated by the compiler as needed
    ...
    > *rule of zero* either defined all or none for move and copy operators and constructor

- to complement =default, we have =delete to indicat that an operation is not to be generated
    > A base class object should not be copied because derived objects may contain additional data not present in the base class. Copying a derived object into a base object copies only the base part (object slicing), causing the derived part to be lost.
    ```c++
        void copy(Shape& s1, const Shape& s2)
    ```

### Conversions
- `complex z1 = 3.14;  // z1 becomes {3.14,0.0}`  this  is implicit conversion
...
- rvalue and lvalue 
    ```c++
        ...
        // x + y produces a temporary result, so it is an rvalue.
    ```
    > The **&&** means “rvalue reference” and is a reference to which we can bind an rvalue , int&& a = 10; a binds to that temporary Lifetime of temporary extends to lifetime of a , a → 10. So an rvalue is – to a first approximation – a value that you can’t assign to, such as an integer returned by a function call.Thus, an rvalue reference is a reference to something that nobody else can assign to, so we can safely “steal” its value.

    - RVO: Compiler eliminates copy/move when returning a temporary object.
    ```c++
        Vector create() {
            return Vector(100);   // RVO
        }
    ```

    - NRVO: NRVO: Compiler may eliminate copy/move when returning a named local object, but it is not guaranteed by the C++ standard.
    ```c++
        Vector create() {
            Vector temp(100);
            return temp;   // NRVO (optional)
        }
        // NRVO cannot apply because two different objects may be returned.
        Vector create(bool flag) {
            Vector a(100);
            Vector b(200);
...
### Resource Management
-  In C++, you can plug in a garbage collector. However, I consider garbage collection the last choice after cleaner, more general, and better localized alternatives to resource management have been exhausted.
...
> A good resource management system handles all kinds of resources. Leaks must be avoided in any long-running system, but excessive resource retention can be almost as bad as a leak.
    ...
    - Real-world problems - Holding resources too long can cause:
        - memory pressure
        - lock contention
